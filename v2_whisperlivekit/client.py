# WhisperLiveKit 서버 연동 파이보 클라이언트
...
# 갤럭시북 서버 주소 (WhisperLiveKit 서버)
SERVER_HOST = "172.20.10.5"  # 갤럭시북 IP
SERVER_PORT = 9090
...
def main():
    ...
    server_url = f"ws://{SERVER_HOST}:{SERVER_PORT}/asr"
    ...
    try:
        if not client.connect():
            ...
            print("갤럭시북에서 서버가 실행 중인지 확인하세요:")
            print("  python galaxy_server_whisperlivekit_v2.py")
            return
        ...
        while True:
            ...
            if text:
                ...
                if response:
                    audio_file = os.path.join(PATH_AUDIO_DIR, "response.mp3")
